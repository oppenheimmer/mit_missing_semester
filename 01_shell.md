...
A path on the shell is location of directory delimited by `/` (on Linux and macOS). On Linux and macOS, the `/` is the *root* of the filesystem, under which all directories and files lie. A path that starts with `/` is called an *absolute* path.

Conversely, any other path which depends on current working directory (instead of the top-level `/`) is a *relative* path. Working directories can be viewed by  `pwd` and changed by `cd` commands respectively.
...
We can use `>>` to append to a file. Where this kind of input/output redirection really shines is in the use of *pipes*. The `|` operator lets you chain programs such that the output of one is the input of the next:
